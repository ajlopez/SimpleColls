...
function MapEnumerator(fn, enumerator) {
    this.next = function (keyvalue) {
        var item = enumerator.next(keyvalue);
        ...
        if (keyvalue)
            newitem = { value: fn(item.value, item.key), key: item.key };
        else
            newitem = fn(item);
...
function Collection(coll) {
    ...
    this.each = function (fn) {
        for (var item = this.next(); item != null; item = this.next())
            fn(item);
            
        return this;
    }
    
    this.reduce = function (fn, initial) {
        var result = initial;
    ...
    this.transduce = function (fntrans, fnred, initial) {
        var result = initial;
        
        for (var item = this.next(); item != null; item = this.next())
            result = fnred(result, fntrans(item));
    ...
    this.map = function (fn) {
        enumerator = new MapEnumerator(fn, enumerator);
        return this;
    }
}
